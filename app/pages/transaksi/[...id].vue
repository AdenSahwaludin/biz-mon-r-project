  ...
  <div class="max-w-2xl mx-auto">
    ...
    <div v-else-if="trx" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div class="p-6 border-b border-gray-200">
        ...
        <div class="space-y-3">
          <div v-for="item in trx.details" :key="item.id" class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-900">{{ item.product?.name || 'Produk' }}</p>
      ...
      <div class="p-6 border-t border-gray-200 flex flex-col sm:flex-row gap-3 no-print">
        <button @click="handleBack" class="flex-1 py-2.5 px-4 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
          ← Kembali
        </button>
    ...
    <div v-else class="bg-white rounded-xl border border-gray-200 py-16 text-center no-print">
      ...
      <button @click="handleBack" class="inline-block mt-3 text-sm text-primary-600 hover:text-primary-700 font-medium">
...
onMounted(async () => {
  ...
  try {
    const res = await fetchWithAuth<any>(`/transactions/${encodeURIComponent(id)}`)
    if (res.success) {
      trx.value = res.data
    } else {
...
async function doDelete() {
  ...
  try {
    const res = await fetchWithAuth<any>(`/transactions/${encodeURIComponent(trx.value.id)}`, {
      method: 'DELETE'
    })
    if (res.success) {
