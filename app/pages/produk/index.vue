  ...
  <div>
    ...
    >
      <div class="flex items-center gap-2.5">
        ...
        <span class="text-xs sm:text-sm font-semibold">Produk dipilih</span>
      </div>
      ...
      <div class="flex items-center gap-2 flex-wrap">
        <button
          ...
          type="button"
          class="px-3.5 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-bold text-xs rounded-lg transition-colors flex items-center gap-1.5 shadow-sm cursor-pointer"
        >
        ...
        <button
          ...
          type="button"
          class="px-3 py-1.5 bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs rounded-lg transition-colors flex items-center gap-1.5 shadow-sm cursor-pointer"
          title="Hapus seluruh produk yang dipilih secara permanen"
        >
        ...
        <button
          ...
          type="button"
          class="px-2.5 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs rounded-lg transition-colors cursor-pointer"
        >
          Batal Pilihan
        </button>
      </div>
          ...
          >
            ...
            <td class="py-3.5 px-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <button
                  v-if="!prod.barcode"
                  @click="scanSingleProductBarcode(prod)"
                  type="button"
                  class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-md text-xs font-semibold transition-colors cursor-pointer"
                  title="Scan Barcode untuk produk ini"
                >
        ...
        <div class="flex items-center justify-between pt-2 border-t border-gray-100">
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              @click="scanSingleProductBarcode(prod)"
              type="button"
              class="inline-flex items-center gap-1 px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-md text-xs font-semibold transition-colors cursor-pointer"
              title="Scan Barcode untuk produk ini"
            >
              <Barcode class="w-3.5 h-3.5" />
              <span>Scan Barcode</span>
    ...
    <div v-if="showBulkDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl max-w-md w-full p-6 space-y-4">
        <h3 class="text-lg font-bold text-gray-900">Hapus {{ selectedProductIds.length }} Produk</h3>
        <p class="text-sm text-gray-600">
          Apakah Anda yakin ingin menghapus <strong>{{ selectedProductIds.length }} produk</strong> yang dipilih? Produk yang pernah digunakan dalam transaksi tidak dapat dihapus dan akan dilewati otomatis.
        </p>
        <div class="flex justify-end gap-3 pt-2">
          <button
          ...
          <button
            ...
            :disabled="isSaving"
            type="button"
            class="px-4 py-2 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 rounded-lg disabled:opacity-50 inline-flex items-center gap-2"
          >
            <Trash2 class="w-4 h-4" />
            <span>{{ isSaving ? 'Menghapus...' : 'Hapus Masal' }}</span>
          </button>
        </div>
      </div>
    </div>
...
const selectedProducts = computed(() => {
  if (selectedProductIds.value.length === 1) {
    ...
    // If no product selected (0 selected), return all products without barcode
    const unbarcoded = filteredData.value.filter((p) => !p.barcode)
    return unbarcoded.length > 0 ? unbarcoded : products.value.filter((p) => !p.barcode)
  }
})
...
function scanSingleProductBarcode(prod: any) {
  selectedProductIds.value = [prod.id]
  openBulkScanner()
}
...
async function doBulkDelete() {
  ...
  isSaving.value = true
  try {
    const res = await fetchWithAuth<any>('/products/batch-delete', {
      method: 'POST',
      body: { ids: selectedProductIds.value }
    })
    if (res && res.success) {
      toast.success(res.message || `${res.data?.deletedCount || 0} produk berhasil dihapus`)
      if (res.data?.blockedCount > 0) {
        toast.warning(`${res.data.blockedCount} produk tidak dihapus karena memiliki riwayat transaksi: ${res.data.blockedProducts.join(', ')}`)
      }
      clearSelection()
    ...
    toast.error(e.data?.message || e.message || 'Gagal menghapus produk terpilih')
  } finally {
    isSaving.value = false
  }
}
